    	...
    	lastVehicle = vehFront
          ...
          if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
            ...
            OpenCoffreInventoryMenu(GetVehicleNumberPlateText(vehFront),Config.VehicleLimit[class])
				   ...
				   ESX.ShowNotification('Der Kofferraum ist geschlossen')
			  end
			end
		else
...
function OpenCoffreInventoryMenu(plate,max)
  ...
  ESX.TriggerServerCallback('esx_truck:getInventoryV', function(inventory)
    ...
    local elements = {}
    table.insert(elements, {label = 'Einpacken', type = 'deposer', value = 'deposer'})
    table.insert(elements, {label = 'Schwarz Geld ' .. inventory.blackMoney, type = 'item_account', value = 'black_money'})
    ...
    ESX.UI.Menu.Open(
      ...
      {
        title    = plate .. ' - ' .. (inventory.weight/1000) .. " / "..(max/1000).." Kg",
        align    = 'top-left',
        elements = elements,
      },
      function(data, menu)
...
function OpenPlayerInventoryMenu(owner,plate,max,weight)
  ...
  ESX.TriggerServerCallback('esx_truck:getPlayerInventory', function(inventory)
    ...
    local elements = {}
    table.insert(elements, {label = 'Zurück', type = 'retour', value = 'retour'})
    table.insert(elements, {label = 'Schwarz Geld ' .. inventory.blackMoney, type = 'item_account', value = 'black_money'})
    ...
    ESX.UI.Menu.Open(
      ...
      {
        title    = plate .. ' - ' .. (weight/1000) .. " / "..(max/1000).." Kg",
        align    = 'top-left',
        elements = elements,
      },
      function(data, menu)
        ...
        if data.current.type == 'item_weapon' then
          ...
          TriggerServerEvent('esx_truck:putItem', plate, data.current.type, data.current.value, data.current.ammo,max)
